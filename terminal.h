...
#include <unistd.h>   // Terminal okuma fonksiyonları
#include <pcap.h>     // Ağ paketlerini yakalamak için
#include <unistd.h>

#include "network_scan.h"
#include "monitor_mode_scan.h"
#include "printing.h"
#include "ascıı.h"
#include "errors.h"
#include "logger.h"
...
int terminal_code(int argc, char const *argv[])
{
    struct termios old;
    ...
    while (1) 
    {
        ...
        else if (strstr(command, " -Ps\0") != NULL) {
            char command_for_mac[MAX_COMMAND_LENGTH];
            strncpy(command_for_mac, command, MAX_COMMAND_LENGTH - 1);
            command_for_mac[MAX_COMMAND_LENGTH - 1] = '\0';
        
            char *value = strstr(command_for_mac, "-Ps");
            if (value && strlen(value) >= 3) {
                memmove(value, value + 3, strlen(value + 3) + 1);
            }
        ...
        else if (strstr(command,"help") != NULL) 
        {
                ...
                {"<network IP> -Sc", ": Scan network"},
                {"<network IP> -Ps", ": Scan Port"},
                ...
                {"version", ": Show program version"} // Yeni komut 
             };
        ...
        else if (i > 0) 
        {
            ...
            log_message(UNKNOWN_COMMAND_WARNING);
